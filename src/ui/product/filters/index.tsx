...
type FilterOption = {
	value: string;
	label: string | CustomFilterType;
	count?: string;
};

type CustomFilterType = { _type: "rating"; rating: number };

const RenderCustomFilterType = ({
	filterType,
}: {
	filterType: FilterOption["label"];
}) => {
	if (typeof filterType === "string") {
		return filterType;
	}

	return <RatingOption rating={filterType.rating} />;
};
				...
				{(option) => (
					<label class="flex items-center gap-2 cursor-pointer group">
						...
						<span class="text-sm text-gray-600 group-hover:text-gray-900 transition">
							<RenderCustomFilterType filterType={option.label} />
						</span>
						{option.count && (
